...
SourceFile(io::IOBuffer) = SourceFile(io.data)
...
struct FileRangeIterator
    sf::SourceFile
    lines::UnitRange{Int}
end

function Base.iterate(fri::FileRangeIterator, i=1)
    i > length(fri.lines) && return nothing
    endline = fri.lines[i]+1
    range = fri.sf.offsets[fri.lines[i]]:(endline > length(fri.sf.offsets) ? endof(fri.sf.data) : fri.sf.offsets[endline])
    (range, String(fri.sf[fri.lines[i]])), i+1
end
